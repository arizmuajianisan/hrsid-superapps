...
export function useApi() {
  ...
  function buildFetchOptions(authHeaders: Record<string, string>, options: FetchOptions) {
    return {
  ...
  async function apiFetch<T>(path: NitroFetchRequest, options: FetchOptions = {}): Promise<T> {
    ...
    try {
      ...
      if (fetchErr.response?.status === 401) {
        try {
          ...
          return await $fetch<T>(path, buildFetchOptions(refreshedHeaders, options))
        } catch {
          auth.clearAuth()
